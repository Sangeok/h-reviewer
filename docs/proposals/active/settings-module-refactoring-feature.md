...
migrated-from: "docs/conventions/settings-module-refactoring-feature.md"
migration-note: "일반 코딩 규칙이 아니라 특정 Settings 리팩터링 변경안이므로 proposals로 분류한다. 내용과 검증 조건은 보존한다."
---

> 이관 메모 (2026-09-06): 일반 코딩 규칙이 아니라 특정 Settings 리팩터링 변경안이므로 proposals로 분류한다. 내용과 검증 조건은 보존한다. 경로·메타데이터 변경 전의 hash와 검토 영수증은 당시 기록이며 새 검증 결과가 아니다.

# Settings Module Refactoring

7개 프론트엔드 스킬 기준 `features/settings` 분석 결과. 22건 이슈 (Critical 3, Important 13, Nice-to-Have 6).
...
```
# 생성 파일
features/settings/hooks/use-user-profile.ts        — 프로필 조회/수정
features/settings/hooks/use-connected-repositories.ts — 저장소 목록/삭제
```
...
### I-11. 저장소 삭제 로직 features/repository로 이동 (coupling)
`deleteWebhook`, `decrementRepositoryCount` 호출을 `features/repository/actions/`로 위임. settings 액션은 repository 액션 호출만 수행.
...
actions는 절대경로(`@/features/settings/actions`), constants는 상대경로(`../../constants`). 모듈 내부는 상대경로로 통일.
...
| 파일 | 변경 유형 |
|------|----------|
| `features/settings/actions/index.ts` | 에러 처리 통일, 반환 타입, 네이밍, Zod 검증 |
| `features/settings/ui/parts/profile-form.tsx` | 훅 추출, 매직 넘버, import 경로 |
| `features/settings/ui/parts/repository-list.tsx` | 훅 추출, 데드 코드 제거, 컴포넌트 분리 |
| `features/settings/hooks/use-user-profile.ts` | **신규** |
| `features/settings/hooks/use-connected-repositories.ts` | **신규** |
| `features/settings/types/index.ts` | **신규** |
| `features/settings/ui/parts/repository-item.tsx` | **신규** |
| `features/settings/constants/index.ts` | 쿼리 키 상수 추가 |
| `features/settings/index.ts` | constants re-export |
| `features/repository/actions/index.ts` | 저장소 삭제 로직 수용 |
| `features/ai/actions/review-pull-request.ts` | 반환 타입 변경 대응 |
| `features/ai/actions/generate-pr-summary.ts` | 반환 타입 변경 대응 |
| `inngest/functions/summary.ts` | I-7: import 경로 `@/features/settings/constants` → `@/features/settings` |
| `inngest/functions/review.ts` | I-7: import 경로 `@/features/settings/constants` → `@/features/settings` |
| `shared/constants/index.ts` | I-7: import 경로 `@/features/settings/constants` → `@/features/settings` |
| `lib/github/github-markdown.ts` | I-7: import 경로 `@/features/settings/constants` → `@/features/settings` |
