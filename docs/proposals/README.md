...
이 폴더의 경로는 저장소 루트 기준 `docs/proposals/`다. 위 상대 경로 명령은 `docs/`에서 실행하거나, 루트에서는 `rg --files --no-ignore docs/proposals/active docs/proposals/completed`를 사용한다. 문서 추가·이동 후 `node scripts/check-docs.mjs`로 검사한다.

HReviewer에서는 기능 명세와 실제 task queue가 있는 로드맵도 proposal로 관리한다. 기존 이관 문서의 미상 날짜·승인 정보와 본문 구조는 [문서 안내의 이관 규칙](../README.md)에 따라 보존한다. 과거 archive의 미완료 문서는 실행 전 코드 재대조가 필요한 pending 문서이며, 이관 자체가 새 구현 요청은 아니다.
