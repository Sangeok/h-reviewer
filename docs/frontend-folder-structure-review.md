...
- **P0: “화면 컴포넌트의 정착지”를 하나로 통일(가장 효과 큼)**
  - **옵션 A(라우트 코로케이션)**: 화면 UI를 라우트 가까이에 둠
    - 예: `app/dashboard/_components/*` 또는 `app/dashboard/(routes)/settings/_components/*`
    - 장점: 라우트 기준 탐색 최강, 페이지 수정할 때 이동거리 최소
    - 단점: 도메인별 재사용 UI가 많아지면 추출 기준이 필요
  - **옵션 B(기능 모듈 중심)**: 화면 UI까지 `module/<feature>/ui/*`로 통일
    - 장점: 기능 단위로 응집(설계가 깔끔), 서버 액션/훅/UI를 한 덩어리로 관리
    - 단점: 라우트 ↔ 기능 매핑 규칙을 팀이 공유해야 함
  - **권장**: 지금 구조는 `module/`이 이미 강하므로 **옵션 B**가 자연스럽지만, 팀 선호/규모에 맞춰 A/B 중 하나를 “규칙”으로 고정하는 게 핵심.
- **P1: 레이어 책임 분리**
  - `module/<feature>/actions`에는 서버 액션/뮤테이션만, UI는 `module/<feature>/components` 또는 `module/<feature>/ui`로 분리
  - 특히 `module/settings/actions/components`는 우선 정리 후보
- **P1: 네이밍 규칙 고정**
  - 라우트 경로와 폴더명을 1:1로 맞추기(복수/단수, 하이픈 사용 규칙 포함)
  - 예: `dashboard-settings` vs `settings` 중 하나로 통일(일반적으로 라우트와 동일한 `settings`가 직관적)
- **P2: 빈 폴더 처리 정책**
  - 당장 안 쓰면 삭제(가독성↑)
