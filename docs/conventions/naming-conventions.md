...
```typescript
// ✅ 좋은 예 (features/dashboard/actions/get-monthly-activity.ts)
...
```typescript
// ✅ features/repository/actions/index.ts
export async function getUserRepositories(page: number, perPage: number) { ... }
export async function connectRepository(owner: string, repo: string) { ... }
export async function disconnectRepository(id: string) { ... }

// ✅ features/repository/hooks/use-repositories.ts
...
// ✅ features/repository/types/index.ts
...
```typescript
// ✅ features/dashboard/actions/get-dashboard-stats.ts
...
// ✅ features/dashboard/actions/get-monthly-activity.ts
const MONTH_NAMES = ["Jan", "Feb", "Mar", ...];
