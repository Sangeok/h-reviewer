...
import { buildGithubArtifactBody } from "./github-artifact-body";
import type { PostedGithubArtifact } from "./github-review-artifacts";
...
export async function postReviewComment(
  ...
  const postedAt = new Date(data.created_at);
  if (Number.isNaN(postedAt.getTime())) {
    throw new Error("GitHub issue comment response has an invalid timestamp");
  }
  if (data.id === null || data.id === undefined) {
    throw new Error("GitHub issue comment response is missing its ID");
  }
